...
public class MainActivity extends Activity {
    ...
    private void scan(boolean replay){List<MarketClient.Contract>x=chosen();if(x.isEmpty()){toast("请先选择合约");return;}savePrefs();status.setText(replay?"正在回放最近30个交易日…":"正在扫描…");io.submit(()->{int count=0,errors=0;for(var c:x){try{var bars=MarketClient.bars(c.code());if(replay){var ss=MarketClient.replay(bars,c,num(period,144),30);count+=ss.size();for(var s:ss)addSignal(s,"回放",false);}else{var s=MarketClient.latestCross(bars,c,num(period,144),closed.isChecked());if(s!=null){count++;addSignal(s,"监控",true);}}}catch(Exception e){errors++;}int done=count,err=errors;runOnUiThread(()->status.setText("处理中：信号 "+done+"，失败 "+err));}int total=count,err=errors;runOnUiThread(()->status.setText((replay?"回放":"扫描")+"完成："+total+" 个信号，"+err+" 个失败"));});}
