...
public class MainActivity extends Activity {
    ...
    private LinearLayout contractsBox, signalsBox; private TextView status;
    ...
    private void buildUi(){
        ...
        LinearLayout a1=new LinearLayout(this);actions.addView(a1);monitor=button("开始监控",v->toggleMonitor());a1.addView(monitor,new LinearLayout.LayoutParams(0,dp(52),1));a1.addView(button("立即扫描",v->scan(false)),new LinearLayout.LayoutParams(0,dp(52),1));
        LinearLayout a2=new LinearLayout(this);actions.addView(a2);a2.addView(button("刷新主力",v->refresh()),new LinearLayout.LayoutParams(0,dp(52),1));a2.addView(button("历史回放30日",v->scan(true)),new LinearLayout.LayoutParams(0,dp(52),1));
        ...
        root.addView(text("穿越信号",19));signalsBox=new LinearLayout(this);signalsBox.setOrientation(LinearLayout.VERTICAL);root.addView(signalsBox);
    ...
    private void scan(boolean replay){List<MarketClient.Contract>x=chosen();if(x.isEmpty()){toast("请先选择合约");return;}savePrefs();status.setText(replay?"正在回放最近30个交易日…":"正在扫描…");io.submit(()->{int count=0,errors=0;String firstError="";for(var c:x){try{var bars=MarketClient.bars(c.code());if(replay){var ss=MarketClient.replay(bars,c,num(period,144),30);count+=ss.size();for(var s:ss)addSignal(s,"回放",false);}else{var s=MarketClient.latestCross(bars,c,num(period,144),closed.isChecked());if(s!=null){count++;addSignal(s,"监控",true);}}}catch(Exception e){errors++;if(firstError.isEmpty())firstError=c.code()+": "+e.getMessage();}int done=count,err=errors;runOnUiThread(()->status.setText("处理中：信号 "+done+"，失败 "+err));}int total=count,err=errors;String detail=firstError;runOnUiThread(()->{status.setText((replay?"回放":"扫描")+"完成："+total+" 个信号，"+err+" 个失败");if(err>0)toast("首个错误："+detail);} );});}
    private void addSignal(MarketClient.Signal s,String type,boolean notify){runOnUiThread(()->{String key=s.code()+s.time()+s.direction();TextView v=text(type+"  "+s.time().format(TF)+"  "+s.code()+" "+s.direction()+"\n收盘 "+String.format(Locale.CHINA,"%.2f",s.close())+" / MA "+String.format(Locale.CHINA,"%.2f",s.ma()),15);v.setTextColor(s.direction().equals("上穿")?Color.rgb(198,40,40):Color.rgb(22,128,60));signalsBox.addView(v,0);if(notify&&sound.isChecked()&&notified.add(key))notifySignal(s);});}
    private void toggleMonitor(){if(timer!=null){timer.shutdownNow();timer=null;monitor.setText("开始监控");status.setText("监控已停止");return;}int mins=num(interval,5);timer=Executors.newSingleThreadScheduledExecutor();timer.scheduleWithFixedDelay(()->runOnUiThread(()->scan(false)),0,mins,TimeUnit.MINUTES);monitor.setText("停止监控");status.setText("监控已启动，每 "+mins+" 分钟扫描");}
